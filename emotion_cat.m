...
folder_objects = fullfile(pwd, 'Stimuli', 'neutral');
folder_faces = fullfile(pwd, 'Stimuli', 'emotive');

filenames_objects = dir(fullfile(folder_objects, '*.png'));
filenames_faces = dir(fullfile(folder_faces, '*.png'));

n_objects = size(dir(fullfile(folder_objects, '*.png')), 1);
n_faces = size(dir(fullfile(folder_faces, '*.png')), 1);
...
%     temp_alpha = ~(sum(temp_image, 3) == 0) * 255;
